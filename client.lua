...
RegisterNetEvent('EmergScripts:WeedExample')
AddEventHandler('EmergScripts:WeedExample', function()
    -- Votre code ici
    print("L'événement EmergScripts:WeedExample a été déclenché.")
    -- Vous pouvez ajouter le code que vous souhaitez exécuter lorsque cet événement est déclenché
end)

-- function Draw3DText(x, y, z, text, scl, font, r, g, b, a, outlineSize, outlineColorR, outlineColorG, outlineColorB, outlineColorA)
--     local onScreen, _x, _y = World3dToScreen2d(x, y, z)
--     local px, py, pz = table.unpack(GetGameplayCamCoords())
--     local dist = GetDistanceBetweenCoords(px, py, pz, x, y, z, 1)
--     local scale = (1 / dist) * scl
...
--         -- Dessiner la boîte de contour
--         local outlineWidth = 1 * scale
--         DrawRect(_x, _y + 0.0125, 0.015 + #text / 370, 0.03, r, g, b, a)
--     end
-- end